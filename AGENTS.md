...
- **Brand palette is white/black/red.** Red (`--color-brand` / `--color-brand-dark` tokens in `src/styles/global.css`) is an accent color only — logo, link hovers, card hover borders — never a background or text color for content. Grays are the `neutral-*` scale (not `slate-*`, which clashes with red).
...
- **Blog is in the nav** (added 2026-08-25 at the user's request, before real posts exist) — the listing shows an empty-state message until posts are published.
